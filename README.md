...
```
project_AI/
├── app.py                  # Streamlit app (3 pages: Predict, Compare, About)
├── train.py                # Offline training script
├── evaluate.py             # Evaluation + chart generation
├── requirements.txt
├── Procfile
├── src/
│   ├── data_parser.py      # PHEME loader, tree builder, early-window slicer
│   ├── feature_extractor.py# 4 feature groups → flat feature vector
│   ├── model.py            # LR / RF / XGBoost + LOEO CV
│   └── explainer.py        # SHAP wrapper + waterfall/bar charts
├── data/
│   └── pheme_processed.pkl # Pre-parsed threads (generated by train.py)
├── models/
│   ├── xgb_model_N5.joblib
│   ├── xgb_model_N10.joblib
│   └── xgb_model_N20.joblib
└── results/
    ├── cv_results.pkl
    └── plots/
```
